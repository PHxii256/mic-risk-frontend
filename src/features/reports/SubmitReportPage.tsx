...
function reportSchema(isAdmin: boolean) {
  return evaluationSchema.extend({
    ...
    assignedDepartment: z.preprocess(
...
export function SubmitReportPage() {
    ...
    createReport.mutate(
      {
        ...
        assignedDepartment: isAdmin ? parsed.assignedDepartment : null,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      <form className="space-y-4" noValidate onSubmit={handleSubmit(onSubmit)}>
        <Card>
          ...
          <CardBody className="space-y-4">
            ...
            >
              ...
              >
                ...
                {RISK_CATEGORIES.map((category) => (
                  <option key={category} value={category}>{t(`riskCategory.${category}`)}</option>
                ))}
              </Select>
            </Field>
            ...
            {isAdmin ? (
              <Field htmlFor="assignedDepartment" label={t('report.assignedDepartment')} optionalLabel={t('form.optional')}>
                <Select id="assignedDepartment" {...register('assignedDepartment')}>
        ...
        {isAdmin ? (
        <Card>
          <CardHeader><CardTitle>{t('report.ownershipAndDueDate')}</CardTitle></CardHeader>
          <CardBody className="space-y-4">
            <Field htmlFor="dueDate" label={t('report.dueDate')} hint={t('report.dueDateHint')} required error={errors.dueDate ? t('form.required') : undefined}>
              <Input id="dueDate" type="date" aria-required="true" {...register('dueDate')} />
            </Field>

            <div className="space-y-2">
              <p className="text-xs font-medium text-ink-muted">{t('report.riskOwners')} <span className="text-danger">*</span></p>
              {owners.fields.map((field, index) => (
                <div key={field.id} className="flex gap-2">
                  <Input
                    type="email"
                    aria-label={t('report.riskOwnerNumber', { number: index + 1 })}
                    aria-invalid={errors.owners?.[index]?.email ? true : undefined}
                    {...register(`owners.${index}.email`)}
                    onFocus={(event) => {
                      if (event.currentTarget.value === '@mohins.com') event.currentTarget.setSelectionRange(0, 0)
                    }}
                  />
                  {owners.fields.length > 1 ? (
                    <Button type="button" variant="ghost" size="sm" aria-label={t('report.removeOwner')} onClick={() => owners.remove(index)}>
                      <Trash2 className="size-3.5" aria-hidden="true" />
                    </Button>
                  ) : null}
                </div>
              ))}
              {errors.owners ? <p className="text-xs text-danger">{t('report.validOwnerRequired')}</p> : null}
              <Button type="button" variant="secondary" size="sm" onClick={() => owners.append({ email: '@mohins.com' })}>
                <Plus className="size-3.5" aria-hidden="true" /> {t('report.addOwner')}
              </Button>
            </div>

            <Field htmlFor="reminderTemplateId" label={t('emailTemplate.template')} optionalLabel={t('form.optional')}>
              <div className="flex flex-wrap items-center gap-2">
                <Select id="reminderTemplateId" className="w-auto min-w-0 flex-1" {...register('reminderTemplateId')}>
            ...
            <label className="flex items-center gap-2 text-sm text-ink">
              <input type="checkbox" className="size-4 accent-accent" {...register('sendReminderEmails')} />
      ...
      {templatesOpen ? <EmailTemplatesDialog onClose={() => setTemplatesOpen(false)} /> : null}
    </div>
  )
}
