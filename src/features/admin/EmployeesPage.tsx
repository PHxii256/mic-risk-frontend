...
export function EmployeesPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {panel.kind === 'create' ? (
        <EmployeeForm onClose={() => setPanel({ kind: 'none' })} />
      ) : null}
      {panel.kind === 'edit' ? (
                    ...
                    <div className="flex justify-end gap-1">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (isSelf && employee.active && !confirm(t('employee.confirmSelfDeactivate')))
                            return
...
function EmployeeForm({ employee, onClose }: { employee?: Employee; onClose: () => void }) {
  ...
  function submit(event: React.FormEvent) {
    ...
    if (isEditing) {
      update.mutate(
        { id: employee.id, name, deptId: Number(deptId), active },
        { onSuccess: onClose },
      )
    } else {
      create.mutate(
        { email, password, name, deptId: Number(deptId), role },
        { onSuccess: onClose },
      )
    }
  }
          ...
          {!isEditing ? (
            <>
              ...
              <Field
                ...
                label={t('employee.initialPassword')}
